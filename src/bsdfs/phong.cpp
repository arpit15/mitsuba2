...
class Phong final : public BSDF<Float, Spectrum> {
public:
    MTS_IMPORT_BASE(BSDF, m_flags, m_components)
    MTS_IMPORT_TYPES(Texture)

    Phong(const Properties &props) : Base(props) {

        m_exponent = props.float_("exponent", 30.f);
        m_diffuse_reflectance  = props.texture<Texture>("diffuse_reflectance", 0.5f);

        if (props.has_property("specular_reflectance"))
            m_specular_reflectance = props.texture<Texture>("specular_reflectance", 0.2f);
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        if (any_or<true>(sample_specular)) {
            Vector3f R = reflect(si.wi);
            // Float exponent = m_exponent->eval(si, sample_specular).mean();
            ...
            masked(bs.wo, sample_specular) = Frame3f(R).to_world(localDir);
            masked(bs.sampled_component, sample_specular) = 1;
        ...
        if (any_or<true>(sample_diffuse)) {
            ...
            masked(bs.sampled_component, sample_diffuse) = 0;
    ...
    Spectrum eval(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (unlikely(!has_diffuse || !has_specular || none_or<false>(active)))
            return 0.f;

        UnpolarizedSpectrum result(0.f);

        if (has_specular) {
            Float alpha = dot(wo, reflect(si.wi));
        ...
        if (has_diffuse)
            result += m_diffuse_reflectance->eval(si, active);
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        Float prob_diffuse = 0.f, prob_specular = 0.f;

        if (has_diffuse) {
            prob_specular = warp::square_to_cosine_hemisphere_pdf(wo);
        }
...
private:
    ...
    ScalarFloat m_specular_sampling_weight, m_exponent;
};

MTS_IMPLEMENT_CLASS_VARIANT(Phong, BSDF)
MTS_EXPORT_PLUGIN(Phong, "Phong")
